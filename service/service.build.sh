...
cmake ${SOURCE_DIR} -DCMAKE_CXX_COMPILER=/usr/bin/${ARCH}-linux-gnu-g++ -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}  \
&& make -j${CONAN_CPU_COUNT} \
&& if [ ${CMAKE_BUILD_TYPE,,} == "release" ]; then ${ARCH}-linux-gnu-strip cplusplus_service; fi \
&& make install;
